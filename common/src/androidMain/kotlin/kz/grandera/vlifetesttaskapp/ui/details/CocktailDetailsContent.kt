...
import kz.grandera.vlifetesttaskapp.ui.effects.verticalFadingEdge
...
internal fun CocktailDetailsContent(component: CocktailDetailsComponent) {
        ...
        if (model.isError) {
            ...
            Surface(color = MaterialTheme.colors.background) {
                Column(modifier = Modifier.fillMaxSize()) {
                    Box {
                        var imageHeight by remember { mutableStateOf(value = 0f) }
                        AsyncImage(
                            modifier = Modifier
                                .fillMaxWidth()
